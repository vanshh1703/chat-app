...
const OfflineChatManager = ({ isOpen, onClose, currentUser }) => {
    ...
    return (
        <div className="fixed inset-0 z-200 flex bg-linear-to-b from-[#8f6a5d] via-[#2b2224] to-black flex-col animate-in slide-in-from-bottom duration-300 text-white">
            {/* Header */}
            <div className="p-4 border-b border-white/10 flex items-center justify-between bg-black/45 backdrop-blur-xl z-10">
                <div className="flex items-center gap-3">
                    <button onClick={mode === null ? handleClose : handleReset} className="p-2 -ml-2 hover:bg-white/10 rounded-xl transition-colors text-white/70">
                        <ArrowLeft size={20} />
                    </button>
                    ...
                    <div>
                        <h2 className="font-bold text-white">Offline Mesh Chat</h2>
                ...
                <button onClick={handleClose} className="p-2 bg-white/10 hover:bg-white/15 rounded-full transition-colors text-white/75">
                    <X size={20} />
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto flex flex-col relative">
                ...
                {mode === null && (
                    <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
                        <div className="w-24 h-24 bg-white/10 rounded-full flex items-center justify-center mb-6 text-blue-300">
                            <Smartphone size={40} />
                        </div>
                        <h3 className="text-2xl font-black text-white mb-2">No Internet? No Problem.</h3>
                        <p className="text-white/65 max-w-sm mb-8">
                        ...
                        <div className="flex flex-col gap-4 w-full max-w-sm">
                            <button onClick={handleStartHost} className="flex items-center justify-between w-full p-5 bg-black/30 border-2 border-white/15 hover:border-blue-300 rounded-2xl transition-all group">
                                <div className="flex items-center gap-4 text-left">
                                    <div className="p-3 bg-white/10 text-blue-300 rounded-xl group-hover:scale-110 transition-transform"><QrCode size={24} /></div>
                                    <div>
                                        <h4 className="font-bold text-white">Host a Chat</h4>
                                        <p className="text-xs text-white/55">Generate a QR code to invite</p>
                                    </div>
                                </div>
                                <ArrowRight className="text-white/45 group-hover:text-blue-300 group-hover:translate-x-1 transition-all" />
                            </button>

                            <button onClick={handleStartJoin} className="flex items-center justify-between w-full p-5 bg-black/30 border-2 border-white/15 hover:border-blue-300 rounded-2xl transition-all group">
                                <div className="flex items-center gap-4 text-left">
                                    <div className="p-3 bg-white/10 text-indigo-300 rounded-xl group-hover:scale-110 transition-transform"><Scan size={24} /></div>
                                    <div>
                                        <h4 className="font-bold text-white">Join a Chat</h4>
                                        <p className="text-xs text-white/55">Scan a friend's QR code</p>
                                    </div>
                                </div>
                                <ArrowRight className="text-white/45 group-hover:text-indigo-300 group-hover:translate-x-1 transition-all" />
                            </button>
                        </div>
                    </div>
                )}
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-md mx-auto w-full">
                        {step === 1 ? (
                            <div className="w-full flex flex-col items-center animate-in fade-in zoom-in duration-300">
                                <h3 className="text-xl font-black text-white mb-2">Step 1: Share Offer</h3>
                                <p className="text-sm text-white/65 mb-8">Ask your friend to select <b>Join</b> and scan this code.</p>
                                ...
                                {localOffer ? (
                                    ...
                                    <div className="w-[240px] h-[240px] flex flex-col items-center justify-center bg-black/35 rounded-3xl mb-8 border border-white/10 animate-pulse">
                                        <div className="w-10 h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
                                        <p className="mt-4 text-xs font-bold text-white/55">Generating Keys...</p>
                                    </div>
                                )}
                            ...
                            <div className="w-full flex flex-col items-center animate-in fade-in zoom-in duration-300">
                                ...
                                <button onClick={handleReset} className="mt-4 text-sm text-white/55 font-bold hover:text-white">Cancel</button>
                            </div>
                        ) : null}
                    </div>
                )}
                ...
                {mode === 'join' && (
                    <div className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-md mx-auto w-full">
                        {step === 1 ? (
                            <div className="w-full flex flex-col items-center animate-in fade-in zoom-in duration-300">
                                <h3 className="text-xl font-black text-white mb-2">Step 1: Scan Offer</h3>
                                <p className="text-sm text-white/65 mb-6">Scan the QR code shown on the host's screen.</p>
                            ...
                            <div className="w-full flex flex-col items-center animate-in fade-in zoom-in duration-300">
                                <h3 className="text-xl font-black text-white mb-2">Step 2: Share Answer</h3>
                                <p className="text-sm text-white/65 mb-8">Show this code to the host so they can scan it.</p>
                                ...
                                <button onClick={() => setStep(1)} className="mt-8 text-sm text-white/55 font-bold hover:text-white">Cancel</button>
                            </div>
                        ) : null}
                    </div>
                )}
                ...
                {/* Chat Mode UI */}
                {mode === 'chat' && (
                    <div className="flex-1 flex flex-col bg-black/55 backdrop-blur-xl absolute inset-0 z-20">
                        {/* Messages Area */}
                            ...
                            {messages.map((msg, i) => {
                                ...
                                return (
                                    <div key={i} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[75%] px-4 py-3 rounded-2xl shadow-sm border ${isMine ? 'bg-blue-600/85 text-white border-blue-300/30 rounded-tr-sm' : 'bg-black/45 text-white border-white/15 rounded-tl-sm'}`}>
                        ...
                        <div className="p-3 bg-black/60 border-t border-white/10">
                            <form onSubmit={sendMessage} className="flex gap-2 items-center bg-black/45 p-1.5 rounded-2xl border border-white/15">
                                <input 
                                    value={messageText} 
                                    onChange={e => setMessageText(e.target.value)} 
                                    placeholder="Type an offline message..." 
                                    className="flex-1 bg-transparent outline-none text-sm px-3 py-2 text-white placeholder:text-white/45" 
                                />

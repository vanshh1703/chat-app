...
import { X, Image as ImageIcon, Video, Music, FileText, Download, Link as LinkIcon, ArrowUpRight, Phone, Mail, Bell, BellOff, ChevronRight, Shield } from 'lucide-react';
import * as api from '../api/api';

// Search icon (not always exported from lucide-react)
...
const MediaPreview = ({ msg }) => {
    ...
    if (msg.message_type === 'audio') return (
        <div className="w-full h-full flex items-center justify-center bg-purple-50 dark:bg-purple-900/20 text-purple-500">
            <Music size={24} />
        </div>
    );
...
const ProfileOrganizer = ({ isOpen, onClose, activeChat, messages, isMuted, onToggleMute, onStartCall, onStartSearch }) => {
    ...
    const [subView, setSubView] = useState('profile'); // 'profile' | 'media'
    const [activeTab, setActiveTab] = useState('media'); // 'media' | 'docs' | 'audio' | 'links'
    const [sharedMedia, setSharedMedia] = useState([]);
    const [isLoadingMedia, setIsLoadingMedia] = useState(false);

    // Sync alias input when activeChat changes
    useEffect(() => {
        setAliasInput(activeChat?.alias || '');
        setEditingAlias(false);
    }, [activeChat]);

    // Fetch shared media whenever panel opens
    useEffect(() => {
    ...
    const { media, docs, audio, links } = useMemo(() => {
        ...
        return {
            ...
            links: validMessages.filter(m => m.message_type === 'text' && m.content?.match(urlRegex)),
        };
    }, [sharedMedia]);
    ...
    const handleSetAlias = async (e) => {
        ...
        try {
            ...
            setEditingAlias(false);
            if (activeChat) activeChat.alias = aliasInput;
        } catch {
        ...
        <div className="flex-1 overflow-y-auto pb-20 custom-scrollbar sm:pb-10">
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                    {[
                        { icon: Phone,  label: 'Call',   color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-900/30', onClick: () => onStartCall('voice') },
                ...
                >
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-1 text-blue-600 font-bold text-xs">
                            {media.length + audio.length + docs.length + links.length} <ChevronRight size={14} />
                        </div>
                    </div>
                    {[...media, ...audio].length > 0 ? (
                        <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                            {[...media, ...audio].slice(0, 5).map((msg, idx) => (
                                <div key={idx} className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl shrink-0 overflow-hidden bg-gray-100 dark:bg-slate-800 border-2 border-white dark:border-slate-700 shadow-sm">
                ...
                <div className="space-y-2 sm:space-y-3 mt-2">
                    ...
                    <button className="w-full flex items-center gap-3 sm:gap-4 p-4 sm:p-5 bg-gradient-to-r from-emerald-50 via-white to-blue-50 dark:from-slate-800 dark:via-slate-900 dark:to-blue-900/10 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-md hover:shadow-lg transition-all">
                        ...
                        <div className="text-left flex-1">
                            <p className="text-sm font-bold text-gray-800 dark:text-slate-200">Privacy</p>
                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Private messaging enabled</p>
                        </div>
                ...
                {activeTab === 'docs' && (
                    <div className="space-y-2">
                        {docs.length > 0 ? docs.map(msg => (
                            <a key={msg.id} href={msg.file_url} download={msg.content} target="_blank" rel="noreferrer"
                    ...
                    <div className="space-y-2">
                        {links.length > 0 ? links.map(msg => {
                            ...
                            return extractedLinks.map((link, idx) => (
                                <a key={`${msg.id}-${idx}`} href={link} target="_blank" rel="noreferrer"
